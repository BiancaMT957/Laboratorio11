...
scan:
	...
	@if which grype >/dev/null 2>&1; then \
	  grype $(IMAGE) --add-cpes-if-none -o sarif --fail-on $(SCAN_FAIL_SEVERITY) > "$(ARTIFACTS)/$(SERVICE)-grype.sarif" || true; \
	else \
	  ...
	  MSYS_NO_PATHCONV=1 MSYS2_ARG_CONV_EXCL="*" $(DOCKER) run --rm -v "$$(pwd)/$(ARTIFACTS)":/in anchore/grype:latest \
	    ...
	    > "$(ARTIFACTS)/$(SERVICE)-grype.sarif" || true; \
	fi
	@echo "GRYPE -> $(ARTIFACTS)/$(SERVICE)-grype.sarif (gate informativo)"
